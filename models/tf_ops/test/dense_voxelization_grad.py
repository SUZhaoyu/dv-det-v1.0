...
from models.tf_ops.loader import grid_sampling, get_gt_bbox, dense_voxelization, dense_voxelization_grad_test
...
from models.tf_ops.test.test_utils import fetch_instance, get_rgbs_from_coors
...
if __name__ == '__main__':
    ...
    input_coors_p, input_features_p, input_num_list_p, input_bbox_p = \
        inputs_placeholder(input_channels=3,
    ...
    voxels, idx = dense_voxelization(input_coors=coors,
    ...
    input_grad = dense_voxelization_grad_test(input_features=features,
                                              output_idx=idx,
                                              grad=voxels)
    ...
    with tf.Session() as sess:
        for _ in tqdm(range(3)):
            ...
            output_coors, output_grad, output_num_list = \
                sess.run([coors, input_grad, num_list],
                           feed_dict={input_coors_p: input_coors,
                                      input_features_p: get_rgbs_from_coors(input_coors, repeat=20),
        ...
        Converter.compile(coors=convert_threejs_coors(fetch_instance(output_coors, output_num_list, id)),
                          intensity=None,
                          default_rgb=fetch_instance(output_grad, output_num_list, id),
                          bbox_params=None,
                          task_name='dense_voxelization_grad')

        Converter.compile(coors=convert_threejs_coors(fetch_instance(input_coors, input_num_list, id)),
                          intensity=None,
                          default_rgb=fetch_instance(get_rgbs_from_coors(input_coors, repeat=20), input_num_list, id),
                          bbox_params=None,
                          task_name='dense_voxelization_grad_input')
