...
REGISTER_OP("BoxesIouOp")
    ...
    .SetShapeFn([](InferenceContext* c){
        ShapeHandle input_boxes_a_shape;
        TF_RETURN_IF_ERROR(c->WithRank(c->input(0), 3, &input_boxes_a_shape));

        ShapeHandle input_boxes_b_shape;
        TF_RETURN_IF_ERROR(c->WithRank(c->input(1), 3, &input_boxes_b_shape));

        DimensionHandle batch_size = c->Dim(input_boxes_a_shape, 0);
        DimensionHandle input_boxes_a_num = c->Dim(input_boxes_a_shape, 1);
        DimensionHandle input_boxes_b_num = c->Dim(input_boxes_b_shape, 1);

        ShapeHandle output_ious_shape = c->MakeShape({batch_size, input_boxes_a_num, input_boxes_b_num});
        c->set_output(0, output_ious_shape);
...
public:
    ...
    void Compute(OpKernelContext* context) override {
        ...
        OP_REQUIRES(context, input_boxes_a.dims()==3 && input_boxes_a.shape().dim_size(2)==7,
                    errors::InvalidArgument("BoxesIouOp expects boxes_a in shape: [B, M, 7]."));

        const Tensor& input_boxes_b = context->input(1);
        auto input_boxes_b_ptr = input_boxes_b.template flat<float>().data();
        OP_REQUIRES(context, input_boxes_b.dims()==3 && input_boxes_b.shape().dim_size(2)==7,
                    errors::InvalidArgument("BoxesIouOp expects boxes_b in shape: [B, N, 7]."));

        const int batch_size = input_boxes_a.dim_size(0);

        const int num_a = input_boxes_a.dim_size(1);
        const int num_b = input_boxes_b.dim_size(1);
        const int num_attr = input_boxes_a.dim_size(2);


        Tensor* output_iou3d = nullptr;
        auto output_iou3d_shape = TensorShape({batch_size, num_a, num_b});
        ...
        for (int b=0; b<batch_size; ++b) {
            boxesIouGPUKernelLauncher(
                ignore_height,
                num_a,
                input_boxes_a_ptr + b * num_a * num_attr,
                num_b,
                input_boxes_b_ptr + b * num_b * num_attr,
                output_iou3d_ptr + b * num_a * num_b);
//        boxesIouGPUKernelLauncher(
//            ignore_height,
//            num_a,
//            input_boxes_a_ptr,
